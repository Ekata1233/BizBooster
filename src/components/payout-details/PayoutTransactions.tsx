...
const PayoutTransactions: React.FC<PayoutTransactionsProps> = ({
  ...
  // ✅ Merge logic for "total"
  const mergedData = useMemo(() => {
    if (activeTab === "user") return payoutData.users || [];
    if (activeTab === "provider") return payoutData.providers || [];
    return [...(payoutData.users || []), ...(payoutData.providers || [])];
  }, [activeTab, payoutData]);
  ...
  const tableData = paginatedData.map((item, index) => ({
    serial: (currentPage - 1) * rowsPerPage + index + 1,
    type: item.type || (item.storeName ? "provider" : "user"),
    name: item.type === "user" ? item.fullName || "-" : item.storeName || "-",
    email: item.email || "-",
    phone: item.type === "user" ? item.mobileNumber || "-" : item.phoneNo || "-",
    balance: item.wallet?.balance?.toFixed(2) || "0.00",
