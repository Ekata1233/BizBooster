...
const ServiceDetailsForm: React.FC<Props> = ({ data, setData, fieldsConfig }) => {
  ...
  const [companyDetails, setCompanyDetails] = useState<CompanyDetailsItem[]>([{ name: '', location: '', details: [{ title: '', description: '' }] }]);
  ...
  const [courseCurriculum, setCourseCurriculum] = useState<CourseCurriculumItem[]>([{ title: '', lists: [''], model: [''] }]);
  ...
  useEffect(() => {
    ...
    setOperatingCities(data.operatingCities || ['']);
    setBrochureImage(data.brochureImage || []);
    setEmiavailable(data.emiavailable || ['']);
    setCounter(data.counter || [{ number: '', title: '' }]);
    setFranchiseOperatingModel(data.franchiseOperatingModel || [{
      info: '',
      title: '',
      description: '',
      features: [{ icon: '', subtitle: '', subDescription: '' }],
    ...
    setKeyAdvantages(data.keyAdvantages || [{ icon: '', title: '', description: '' }]);
    setCompleteSupportSystem(data.completeSupportSystem || [{ icon: '', title: '', lists: [''] }]);
    setTrainingDetails(data.trainingDetails || ['']);
    setAgreementDetails(data.agreementDetails || ['']);
    setGoodThings(data.goodThings || ['']);
    setCompareAndChoose(data.compareAndChoose || ['']);
    setCompanyDetails(data.companyDetails || [{ name: '', location: '', details: [{ title: '', description: '' }] }]);
    setRoi(data.roi || ['']);
    ...
    setEligibleFor(data.eligibleFor || ['']);
    setCourseCurriculum(data.courseCurriculum || [{ title: '', lists: [''], model: [''] }]);
    setCourseIncludes(data.courseIncludes || ['']);
    setCertificateImage(data.certificateImage || []);
    setWhomToSell(data.whomToSell || [{ icon: '', lists: '' }]);
    setInclude(data.include || ['']);
    setNotInclude(data.notInclude || ['']);
    setSafetyAndAssurance(data.safetyAndAssurance || ['']);
  }, [data]);
  ...
  useEffect(() => {
    ...
    const updatedData: ServiceDetails = {
      ...
      connectWith,
      timeRequired,
      extraImages: extraImages.map(item => item.file || item.icon),
      highlightPreviews,
    ...
    // Existing fields
    benefits, aboutUs, highlight, document, assuredByFetchTrue, howItWorks, termsAndConditions, faq,
    extraSections, whyChooseUs, packages, weRequired, weDeliver, moreInfo, connectWith, timeRequired, extraImages,
    // New fields
    operatingCities, brochureImage, emiavailable, counter, franchiseOperatingModel, businessFundamental,
    keyAdvantages, completeSupportSystem, trainingDetails, agreementDetails, goodThings, compareAndChoose,
    companyDetails, roi, level, lessonCount, duration, whatYouWillLearn, eligibleFor, courseCurriculum,
    courseIncludes, certificateImage, whomToSell, include, notInclude, safetyAndAssurance
  ]);
  ...
  const handleFileArrayChange = (event: React.ChangeEvent<HTMLInputElement>, setter: React.Dispatch<React.SetStateAction<(string | File)[]>>) => {
    const files = event.target.files;
    if (files) {
      const fileArray = Array.from(files);
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        {/* ============= NEW EXTENDED FIELDS - ADDED BEFORE PACKAGES ============= */}
        ...
        {fieldsConfig?.operatingCities && (
          <div>
            ...
            {renderArrayField<string>(operatingCities, setOperatingCities, (city, idx, updateCity) => (
              <Input 
                value={city} 
        ...
        {fieldsConfig?.brochureImage && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Brochure Images</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
            <FileInput 
              onChange={(e) => handleFileArrayChange(e, setBrochureImage)} 
              multiple 
            />
            <div className="mt-2">
        ...
        {/* EMI Available */}
        {fieldsConfig?.emiavailable && (
          <div>
            <div className="flex items-center gap-2">
              <Label>EMI Available</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
            {renderArrayField<string>(emiavailable, setEmiavailable, (emi, idx, updateEmi) => (
              <Input 
                value={emi} 
        ...
        {fieldsConfig?.counter && (
          <div>
            ...
            {renderArrayField<CounterItem>(counter, setCounter, (item, idx, updateItem) => (
              <div className="grid gap-2">
                <Input 
                  type="number"
                  value={item.number} 
        ...
        {fieldsConfig?.franchiseOperatingModel && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Franchise Operating Model</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
                ...
                <div className="border p-4 rounded mb-4">
                  ...
                  <div className="mb-3">
                    ...
                    {renderArrayField<{
                      ...
                      model.features,
                      (newFeatures) => {
                        updateModel({ ...model, features: typeof newFeatures === 'function' ? 
                          newFeatures(model.features) : newFeatures });
                      },
                      (feature, featureIdx, updateFeature) => (
                        <div className="grid gap-2 p-2 border rounded mb-2">
                          <FileInput
                  ...
                  <div>
                    ...
                    {renderArrayField<string>(
                      model.tags,
                      (newTags) => {
                        updateModel({ ...model, tags: typeof newTags === 'function' ? 
        ...
        {fieldsConfig?.businessFundamental && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Business Fundamental</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
            <textarea 
              value={businessFundamental.description} 
              placeholder="Description" 
              onChange={e => setBusinessFundamental(prev => ({ ...prev, description: e.target.value }))}
            ...
            {renderArrayField<{ subtitle: string; subDescription: string }>(
              ...
              (newPoints) => {
                setBusinessFundamental(prev => ({ 
                  ...prev, 
                  points: typeof newPoints === 'function' ? newPoints(prev.points) : newPoints 
                }));
              },
              (point, pointIdx, updatePoint) => (
                <div className="grid gap-2">
                  <Input 
        ...
        {fieldsConfig?.keyAdvantages && (
          <div>
            ...
            {renderArrayField<KeyAdvantageItem>(keyAdvantages, setKeyAdvantages, (item, idx, updateItem) => (
              <div className="grid gap-2">
                <FileInput
                  onChange={(e) => handleFileUpload(e, item, updateItem, 'icon')}
                />
        ...
        {fieldsConfig?.completeSupportSystem && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Complete Support System</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
              ...
              (item, idx, updateItem) => (
                <div className="border p-3 rounded mb-3">
                  ...
                  {renderArrayField<string>(
                    item.lists,
                    (newLists) => {
                      updateItem({ ...item, lists: typeof newLists === 'function' ? newLists(item.lists) : newLists });
                    },
                    (list, listIdx, updateList) => (
                      <Input 
                        value={list} 
        ...
        {fieldsConfig?.trainingDetails && (
          <div>
            ...
            {renderArrayField<string>(trainingDetails, setTrainingDetails, (detail, idx, updateDetail) => (
              <Input 
                value={detail} 
        ...
        {fieldsConfig?.agreementDetails && (
          <div>
            ...
            {renderArrayField<string>(agreementDetails, setAgreementDetails, (detail, idx, updateDetail) => (
              <Input 
                value={detail} 
        ...
        {fieldsConfig?.goodThings && (
          <div>
            ...
            {renderArrayField<string>(goodThings, setGoodThings, (thing, idx, updateThing) => (
              <Input 
                value={thing} 
        ...
        {fieldsConfig?.compareAndChoose && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Compare and Choose</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
            {renderArrayField<string>(compareAndChoose, setCompareAndChoose, (item, idx, updateItem) => (
              <Input 
                value={item} 
                placeholder="Compare item" 
                onChange={e => updateItem(e.target.value)} 
              />
            ), '')}
          </div>
        ...
        {fieldsConfig?.companyDetails && (
          <div>
            ...
            {renderArrayField<CompanyDetailsItem>(
              companyDetails, 
              setCompanyDetails, 
              (company, idx, updateCompany) => (
                <div className="border p-4 rounded mb-4">
                  <div className="grid gap-3 mb-3">
                    <Input 
                      value={company.name} 
                  ...
                  {renderArrayField<{ title: string; description: string }>(
                    company.details,
                    (newDetails) => {
                      updateCompany({ ...company, details: typeof newDetails === 'function' ? 
              ...
              { name: '', location: '', details: [{ title: '', description: '' }] }
            )}
          </div>
        )}
        ...
        {fieldsConfig?.roi && (
          <div>
            ...
            {renderArrayField<string>(roi, setRoi, (item, idx, updateItem) => (
              <Input 
                value={item} 
                placeholder="ROI detail" 
                onChange={e => updateItem(e.target.value)} 
              />
            ), '')}
          </div>
        ...
        {fieldsConfig?.duration && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="mb-1">Weeks</Label>
                <Input 
                  type="number"
                  value={duration.weeks || ''} 
                  placeholder="Weeks" 
                  onChange={e => setDuration(prev => ({ ...prev, weeks: e.target.value ? Number(e.target.value) : null }))} 
                />
              </div>
              <div>
                ...
                <Input 
                  ...
                  placeholder="Hours" 
                  onChange={e => setDuration(prev => ({ ...prev, hours: e.target.value ? Number(e.target.value) : null }))} 
                />
              </div>
            </div>
          </div>
        ...
        {fieldsConfig?.whatYouWillLearn && (
          <div>
            ...
            {renderArrayField<string>(whatYouWillLearn, setWhatYouWillLearn, (item, idx, updateItem) => (
              <Input 
                value={item} 
        ...
        {fieldsConfig?.eligibleFor && (
          <div>
            ...
            {renderArrayField<string>(eligibleFor, setEligibleFor, (item, idx, updateItem) => (
              <Input 
                value={item} 
        ...
        {fieldsConfig?.courseCurriculum && (
          <div>
            ...
            {renderArrayField<CourseCurriculumItem>(
              courseCurriculum, 
              setCourseCurriculum, 
              (curriculum, idx, updateCurriculum) => (
                <div className="border p-4 rounded mb-4">
                  <div className="mb-3">
                    <Input 
                  ...
                  <div className="mb-3">
                    ...
                    {renderArrayField<string>(
                      curriculum.lists,
                      (newLists) => {
                        updateCurriculum({ ...curriculum, lists: typeof newLists === 'function' ? 
                  ...
                  <div>
                    ...
                    {renderArrayField<string>(
                      curriculum.model,
                      (newModel) => {
                        updateCurriculum({ ...curriculum, model: typeof newModel === 'function' ? 
        ...
        {fieldsConfig?.courseIncludes && (
          <div>
            ...
            {renderArrayField<string>(courseIncludes, setCourseIncludes, (item, idx, updateItem) => (
              <Input 
                value={item} 
                placeholder="Course inclusion" 
                onChange={e => updateItem(e.target.value)} 
              />
            ), '')}
          </div>
        ...
        {fieldsConfig?.certificateImage && (
          <div>
            ...
            <FileInput 
              onChange={(e) => handleFileArrayChange(e, setCertificateImage)} 
              multiple 
            />
            <div className="mt-2">
        ...
        {fieldsConfig?.whomToSell && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Whom To Sell</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
            {renderArrayField<WhomToSellItem>(whomToSell, setWhomToSell, (item, idx, updateItem) => (
              <div className="grid gap-2">
                <FileInput
                  onChange={(e) => handleFileUpload(e, item, updateItem, 'icon')}
                />
        ...
        {/* ============= NEW FIELDS AFTER MORE INFO SECTION ============= */}
        ...
        {/* ============= CKEDITOR FIELDS (Include, Not Include, Safety and Assurance) ============= */}
        ...
        {fieldsConfig?.safetyAndAssurance && (
          <div>
            <div className="flex items-center gap-2">
              <Label>Safety and Assurance</Label>
              <span className="text-red-500 text-sm font-semibold">(All Services)</span>
            </div>
            {editorReady && (
              <ClientSideCustomEditor
