...
interface LeadData {
  ...
  price: string;
  share_1: string;
  extra_share_1: string;
  share_2: string;
  extra_share_2: string;
  share_3: string;
  extra_share_3: string;
  totalShare: string;
  leadStatus: string;
}
...
const columnsSelfLead = [
  ...
  { header: "Service Name", accessor: "serviceName" },
  {
    ...
    render: (row: any) => {
      const contactParts = row.contactDetails?.split(" | ") || [];
      const [name, email, phone] = contactParts;
      return (
...
const SelfLeadShare3 = ({ userId }: SelfLeadProps) => {
  const [leads, setLeads] = useState<LeadData[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  // ✅ Fetch leads from your new API
  useEffect(() => {
    const fetchLeads = async () => {
      ...
      try {
        ...
        setError("Failed to load self lead data");
      } finally {
        setLoading(false);
      }
    };
  ...
  const mappedData = leads.map((lead, index) => {
    const myCommission = `₹${Number(lead.share_3 || 0).toLocaleString()}`;
    const addOnCommission = `₹${Number(lead.extra_share_3 || 0).toLocaleString()}`;
    const addOnCommissionValue = Number(lead.extra_share_3 || 0);
