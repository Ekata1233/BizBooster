...
const SelfLeadShare2 = ({ userId, isAction }: SelfLeadProps) => {
  ...
  // ✅ Fetch self-lead data using your API
  useEffect(() => {
  ...
  const mappedData = [...checkouts].reverse().map((checkout: any, index: number) => {
    const price = Number(checkout?.price ?? 0);
    const myCommissionValue = Number(checkout?.share_2 ?? 0);
    const addOnCommissionValue = Number(checkout?.extra_share_2 ?? 0);

    const myCommission = `₹${myCommissionValue.toFixed(2)}`;
    const addOnCommission = `₹${addOnCommissionValue.toFixed(2)}`;

    let commissionWithStatus: React.ReactNode = (
      <div className="flex flex-col">
        <span>{myCommission}</span>
        {checkout?.leadStatus === "completed" && (
    ...
    // Parse contactDetails: "Name | email | phone"
    const contactParts = checkout?.contactDetails?.split("|") || [];
    const [name, email, phone] = contactParts.map((s: string) => s.trim());

    return {
      id: checkout._id || checkout.leadId,
      sr: index + 1,
      leadId: checkout?.leadId || "N/A",
      serviceName: checkout?.serviceName || "N/A",
      userName: name || "N/A",
      userEmail: email || "N/A",
      userPhone: phone || "N/A",
      price: `₹${price.toFixed(2)}`,
      commission: commissionWithStatus,
      leadStatus: checkout?.leadStatus || "pending",
    };
  });
