...
const UserWallet = ({ userId }: UserWalletProps) => {
  ...
  const summaryWallet = isWalletAvailable
    ? wallet
    : { balance: 0, totalCredits: 0, totalDebits: 0, transactions: [] };

  const transactions = summaryWallet.transactions || [];

  // Filter transactions by search & tab
  const filteredTransactions = useMemo(() => {
    return transactions
      .filter((txn) => {
        ...
        const matchTab =
          activeTab === 'all' ||
          (activeTab === 'withdraw' ? txn.source === 'withdraw' : txn.type === activeTab);
        return matchSearch && matchTab;
      })
  ...
  const enrichedTransactions = enrichedAscending.reverse(); // newest first

  // Pagination
  const totalPages = Math.ceil(enrichedTransactions.length / rowsPerPage);
  const indexOfLastRow = currentPage * rowsPerPage;
  const indexOfFirstRow = indexOfLastRow - rowsPerPage;
  const paginatedTransactions = enrichedTransactions.slice(indexOfFirstRow, indexOfLastRow);
      ...
      {!isWalletAvailable ? (
        ...
        <>
          ...
          <div className="flex gap-2 mb-4">
            {['all', 'credit', 'debit', 'withdraw'].map((tab) => (
              <button
                key={tab}
                onClick={() => {
                  setActiveTab(tab as 'all' | 'credit' | 'debit' | 'withdraw');
          ...
          {filteredTransactions.length === 0 ? (
            ...
            <>
              <BasicTableOne columns={columnsWallet} data={paginatedTransactions as IWalletTransaction[]} />
