...
export async function GET() { // Changed Request to NextRequest for consistency
  ...
  try {
    const aboutUsEntries = await RefundPolicy.find({}); // This correctly returns an array
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const newAboutUsEntry = await RefundPolicy.create({ content }); // Create a new document
