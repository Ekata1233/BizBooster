...
//       return NextResponse.json({ success: false, message: "Invalid Certification ID" }, { status: 400, headers: corsHeaders });
//     }
...
//       return NextResponse.json({ success: false, message: "Certification not found" }, { status: 404, headers: corsHeaders });
//     }
...
//         return NextResponse.json({ success: false, message: "Invalid video index" }, { status: 400, headers: corsHeaders });
//       }
...
//     // ✅ Case 2: Add new videos (multiple)
//     } else if (videoFiles.length > 0) {
//       if (videoFiles.length !== videoNames.length || videoNames.length !== videoDescriptions.length) {
//         return NextResponse.json({ success: false, message: "Mismatch in new video data" }, { status: 400, headers: corsHeaders });
//       }
...
//       return NextResponse.json({ success: false, message: "Duplicate name" }, { status: 409, headers: corsHeaders });
//     }

//     if ((error as unknown)?.name === "ValidationError") {
//       const messages = Object.values((error as unknown)?.errors).map((err: any) => err.message);
//       return NextResponse.json({ success: false, message: `Validation: ${messages.join(", ")}` }, { status: 400, headers: corsHeaders });
//     }
...
// function hasCode(err: unknown): err is MongoDuplicateError {
//   return typeof err === 'object' && err !== null && 'code' in err;
// }

// function hasValidationErrors(err: unknown): err is MongooseValidationError {
