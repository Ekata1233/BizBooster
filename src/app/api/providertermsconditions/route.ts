...
export async function GET() {
  ...
  try {
    const policies = await ProviderTermsConditions.find().populate({
      path: 'module',
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const existing = await ProviderTermsConditions.findOne({ module: moduleId });
    ...
    // Create new
    const created = await ProviderTermsConditions.create({ module: moduleId, content });
    return NextResponse.json(
      {
        success: true,
        data: created,
        message: 'Provider Terms and Conditions content created successfully.',
      },
      { status: 201, headers: corsHeaders },
    );
  } catch (error: unknown) {
    console.error('POST /api/providertermsconditions error:', error);
    return NextResponse.json(
      {
        success: false,
