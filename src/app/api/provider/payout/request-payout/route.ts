...
export async function GET() {
    ...
    try {
        // 🔹 Fetch only wallets that have positive withdrawable balance
        ...
        for (const wallet of providerWallets) {
            const provider = wallet.providerId;
            const amount = wallet.withdrawableBalance;

            if (!provider || amount <= 0 || provider._id.equals(ADMIN_ID)) continue;

            const beneficiaryId = wallet.beneficiaryId || provider._id.toString();
