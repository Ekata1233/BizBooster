...
//     const { providerId, bankAccount, ifsc, upiId, name, email, phone, skipCashfree } = body;
...
//     // 👇 Only call Cashfree API if skipCashfree is not true
//     // if (!skipCashfree) {
//     //   const cashfreeResponse = await fetch("https://payout-api.cashfree.com/payout/v1/addBeneficiary", {
//     //     method: "POST",
//     //     headers: {
...
//       {
//         success: true,
//         message: skipCashfree
//           ? "Bank details saved locally (Cashfree skipped)"
//           : "Bank details saved & registered with Cashfree",
//       },
//       { status: 200, headers: corsHeaders }
//     );
//   } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    // Create empty wallet with just providerId if it doesn't exist
    const wallet = await ProviderWallet.findOneAndUpdate(
      { providerId },
      {
        $setOnInsert: {
          providerId,
    ...
    return NextResponse.json(
      { success: true, message: "Provider wallet created successfully", wallet },
      { status: 201, headers: corsHeaders }
    );
  } catch (error) {
    console.error("[PROVIDER_WALLET_CREATE_ERROR]", error);
    return NextResponse.json(
      { success: false, message: "Internal server error" },
      { status: 500, headers: corsHeaders }
    );
