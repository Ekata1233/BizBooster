    ...
    for (const wallet of wallets) {
      ...
      const res = await fetch("https://sandbox.cashfree.com/payout/transfers", {
        ...
        body: JSON.stringify({
          ...
          transfer_amount: amount,
          transfer_currency: "INR",
          beneficiary_details: { beneficiary_id: user._id.toString() }, // ⚠️ replace with actual beneficiary id
        }),
      });
      ...
      wallet.transactions.push({
        ...
        method: "BankTransfer",
        source: "payout",
        status: data.status?.toUpperCase() === "RECEIVED" ? "pending" : "failed",
        createdAt: new Date(),
        balanceAfterTransaction: wallet.balance, // ✅ unchanged
        leadId: "",
        commissionFrom: "",
      ...
      // ⚠️ DO NOT deduct balance here — wait for webhook confirmation
