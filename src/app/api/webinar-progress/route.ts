...
export async function POST(req: NextRequest) {
  ...
  const { userId, webinarId } = await req.json();
  ...
  try {
    const progress = await WebinarProgress.findOneAndUpdate(
      { userId, webinarId },
      {
        $inc: { joinCount: 1 },          // increment join count
        isCompleted: true,               // mark attended
      },
