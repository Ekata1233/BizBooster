...
const WebinarDetailPage: React.FC = () => {
  ...
  interface WebinarProgress {
    userId: string;
    joinCount: number;
  }

  const [progress, setProgress] = useState<WebinarProgress[]>([]);

  const loadWebinarProgress = async () => {
    try {
      if (!webinar) return;
      const promises = webinar.user.map(async (entry) => {
        const userId = entry.user._id || entry.user.userId; // fallback
        if (!userId) return null; // skip if still missing

        const res = await axios.get(
          `/api/webinar-progress?webinarId=${id}&userId=${userId}`
        );
        console.log("Progress response for userId", userId, ":", res.data);
        return res.data;
      });

      const results = (await Promise.all(promises)).filter(Boolean);
      setProgress(results.flat());
  ...
  const currentUserId = "68b582994c7691f261d7efc4"; // 👈 Replace with actual user ID from auth/session
  const handleJoinWebinar = async () => {
    if (!webinar) return; // Early return if webinar is null
    try {
      await axios.post("/api/webinar-progress", {
        userId: currentUserId,   // 👈 get this from auth/session
        webinarId: webinar._id,
      });
      console.log("✅ Progress saved");

      // open webinar link
      window.open(webinar.displayVideoUrls[0], "_blank");
    } catch (err) {
      console.error("❌ Failed to save webinar progress", err);
    }
  };
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-blue-50">
                <tr>
                  <th className="px-4 py-3 text-left text-black font-bold uppercase tracking-wider">S.No</th>
                  <th className="px-4 py-3 text-left text-black font-bold uppercase tracking-wider">User ID</th>
                  <th className="px-4 py-3 text-left text-black font-bold uppercase tracking-wider">
                    Attendance
                  </th>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {webinar.user.length ? (
                  webinar.user.map((entry, idx) => (
                    ...
                    //     {(() => {
                    //       const userProgress = progress.find((p) => String(p.userId) === String(entry.user._id));
                    //       console.log('Matched progress record for userKey:', userProgress);

                    //       if (!userProgress) {
                    //         return <span className="text-orange-500 font-semibold">Pending</span>;
                    //       }

                    //       if (userProgress.joinCount >= 3) {
                    //         return <span className="text-green-600 font-semibold">Approved</span>;
                    //       }

                    //       return (
                    //         <span className="text-orange-500 font-semibold">
                    //           Pending ({userProgress.joinCount}/3)
                    //         </span>
                    //       );
                    //     })()}
                    //   </td>
