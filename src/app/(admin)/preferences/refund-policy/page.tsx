...
const AdminAboutUsManagementPage: React.FC = () => {
  ...
  const fetchAboutUsContent = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('/api/refundpolicy'); // Use axios
  ...
  const handleSaveAboutUs = async (dataToSave: { _id?: string; content: string }) => {
    ...
    try {
      ...
      if (dataToSave._id) {
        ...
        response = await axios.put(`/api/refundpolicy/${dataToSave._id}`, { content: dataToSave.content });
      } else {
        // Create new entry
        response = await axios.post('/api/refundpolicy', { content: dataToSave.content });
      }
  ...
  const handleDeleteClick = async (id: string) => {
    ...
    setError(null);
    try {
      const response = await axios.delete(`/api/refundpolicy/${id}`);
      if (response.data.success) {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-xl text-gray-700">Loading Refund Policy content...</p>
      </div>
    );
  }
