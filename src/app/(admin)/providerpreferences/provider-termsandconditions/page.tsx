...
const ProviderTermsConditionsPage = dynamic(
    () => import('@/components/providerpreferences-components/ProviderTermsConditionsPage'),
    {
        ssr: false,
        loading: () => (
...
const AdminProviderTermsConditionsPage: React.FC = () => {
    ...
    const fetchPolicies = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get('/api/providertermsconditions');
    ...
    const handleSave = async (data: { _id?: string; content: string; module: string }) => {
        ...
        try {
            ...
            const res = data._id
                ? await axios.put(`/api/providertermsconditions/${data._id}`, payload)
                : await axios.post(`/api/providertermsconditions`, payload);
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await axios.delete(`/api/providertermsconditions/${id}`);
            fetchPolicies();
        } catch {
  ...
  const currentFormEntry = editingEntry ?? BLANK_ENTRY;
    return (
        <div className="container mx-auto px-4 py-8">
            <h1 className="text-4xl font-bold mb-6 text-center">Manage Provider Terms and Conditions</h1>
            ...
            {error && <p className="bg-red-100 text-red-700 px-4 py-2 mb-4 rounded">{error}</p>}
                ...
                <div className="bg-white p-6 rounded shadow mb-10">
                    <h2 className="text-2xl font-semibold mb-4">
                        {currentFormEntry._id ? 'Edit Entry' : 'Add New Entry'}
                    </h2>
...
export default AdminProviderTermsConditionsPage;
