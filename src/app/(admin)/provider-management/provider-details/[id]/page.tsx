...
const ProviderDetailsPage = () => {
  ...
  useEffect(() => {
    if (providerDetails && Object.keys(providerDetails).length > 0) {
      const selected = (Array.isArray(providerDetails) ? providerDetails : [providerDetails]).find(
        (p: Provider) => p._id === id
      ) || null;
  ...
  const hasStoreInfo = provider.storeInfo && Object.values(provider.storeInfo).some(val => val !== undefined && val !== null && val !== '');
  const hasKycInfo = provider.kyc && Object.values(provider.kyc).some(val => val !== undefined && val !== null && val.length > 0);
              ...
              <div className="grid grid-cols-1 gap-6">
                ...
                <div className="border rounded-lg p-6 shadow-sm bg-gradient-to-br from-yellow-50 to-white">
                  ...
                  {hasStoreInfo ? (
                    ...
                    <div className="text-center py-8 text-red-400 italic">
                      Store information is pending
                    </div>
                  )}
                </div>
